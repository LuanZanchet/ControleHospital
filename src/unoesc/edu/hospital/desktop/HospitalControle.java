...
public class HospitalControle extends JFrame {
	...
	HospitalControle() {
		...
		dtmTabela.setValueAt("Hospital", 0, 1);
		...
		for (Hospital hospital : hospitalDao.listarTodos()) {
			...
			dtmTabela.setValueAt(hospital.getCodigo(), linha, 0);
			dtmTabela.setValueAt(hospital.getNome(), linha, 1);
			dtmTabela.setValueAt(hospital.getTelefone(), linha, 2);
			linha++;
		...
		jbtExcluir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (Internacao internacao : internacaoDao.listarTodos()) {
					if (internacao.getHospital().getCodigo().equals(hospitalDao.listarTodos().get(selecionado - 1).getCodigo())) {
							auxx = 1;
						
					}
				}
				if (auxx == 0) {
					hospitalDao.delete(hospitalDao.listarTodos().get(
							selecionado-1));
				...
				if (auxx == 1) {
					JOptionPane.showMessageDialog(null, "Não pode excluir esse Hospital");
					JOptionPane.showMessageDialog(null, "Existe Internações pendente(s)");
				}
				}
		...
		jbtConsultar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selecionado == 0) {
					...
					JOptionPane.showMessageDialog(null, "Selecione o Hospital que deseja Visualizar");
				} else {
					if (aux == 1)
						selecionado = procurar;
					VisualizarHospital tela = new VisualizarHospital();
		...
		jbtAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selecionado == 0) {
					...
					JOptionPane.showMessageDialog(null, "Selecione o hospital que deseja alterar");
				} else {
					if (aux == 1)
						selecionado = procurar;
					AlterarHospital tela = new AlterarHospital();
					tela.setValor1(selecionado - 1);
					dispose();
				}
			}
