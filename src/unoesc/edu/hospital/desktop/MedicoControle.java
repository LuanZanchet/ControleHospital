...
public class MedicoControle extends JFrame{

	private static final long serialVersionUID = -6496414274075405172L;
	...
	MedicoControle(){
		...
		dtmTabela.setValueAt("Medico", 0, 1);
		...
		for (Medico medico : medicoDao.listarTodos()) {
			...
			dtmTabela.setValueAt(medico.getCodigo(), linha, 0);
			dtmTabela.setValueAt(medico.getNome(), linha, 1);
			dtmTabela.setValueAt(medico.getTelefone(), linha, 2);
			linha++;
        ...
        jbtConsultar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selecionado == 0) {
					...
					JOptionPane.showMessageDialog(null, "Selecione o Medico que deseja Visualizar");
				} else {
					if (aux == 1)
						selecionado = procurar;
					VisualizarMedico tela = new VisualizarMedico();
		...
		jbtAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selecionado == 0) {
					...
					JOptionPane.showMessageDialog(null, "Selecione o Medico que deseja alterar");
				} else {
					if (aux == 1)
						selecionado = procurar;
					AlterarMedico tela = new AlterarMedico();
					tela.setValor1(selecionado - 1);
					dispose();
				}
			}
        ...
        jbtExcluir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selecionado == 0) {
					...
					JOptionPane.showMessageDialog(null,
							"Selecione o Medico que deseja excluir");
				} else {
	
				int auxx =0 ;
				for (Internacao internacao : internacaoDao.listarTodos()) {
					if (internacao.getMedico().getCodigo().equals(medicoDao.listarTodos().get(selecionado - 1).getCodigo())) {
							auxx = 1;
						
					}
				}
				if (auxx == 0) {
					medicoDao.delete(medicoDao.listarTodos().get(
							selecionado-1));
				...
				if (auxx == 1) {
					JOptionPane.showMessageDialog(null, "Não pode excluir esse Medico");
					JOptionPane.showMessageDialog(null, "Existe Internações cuidadas pelo mesmo");
				}
				}
