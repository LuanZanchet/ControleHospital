...
public class InternacaoControle extends JFrame{
	...
	InternacaoControle(){
		...
		for (Internacao internacao : internacaoDao.listarTodos()) {
			...
			dtmTabela.setValueAt(internacao.getCodigo(), linha, 0);
			dtmTabela.setValueAt(internacao.getPaciente().getNome(), linha, 1);
			dtmTabela.setValueAt(internacao.getPaciente().getTelefone(), linha, 2);
			linha++;
  ...
  jbtConsultar.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (selecionado == 0) {
				...
				JOptionPane.showMessageDialog(null, "Selecione a internção que deseja Visualizar");
			} else {
				if (aux == 1)
					selecionado = procurar;
				VisualizarInternacao tela = new VisualizarInternacao();
	...
	jbtAlterar.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (selecionado == 0) {
				...
				if (aux == 1)
					selecionado = procurar;
				AlterarInternacao tela = new AlterarInternacao();
				tela.setValor1(selecionado - 1);
				dispose();
			}
		}
  ...
  jbtExcluir.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (selecionado == 0) {
				...
				JOptionPane.showMessageDialog(null,
						"Selecione a Internacao que deseja excluir");
			} else {

				internacaoDao.delete(internacaoDao.listarTodos().get(
						selecionado-1));
